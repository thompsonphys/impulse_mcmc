...
class CBCInjectionGenerator:
    def __init__(self, config_path):
        ...
        self.duration = get_duration(
            self.cbc_parameters, self.injection_options["f_low"]
        )
    ...
    def initialize_detector_network(self, interferometer):
        ...
        if isinstance(interferometer, list):
            ifo_list = [
                bilby.gw.detector.load_interferometer((inter))
                for inter in interferometer
            ]
        else:
            ifo_list = [bilby.gw.detector.load_interferometer((interferometer))]
    ...
    def generate_injection(self):
        ...
        if self.injection_options["approximant"] == "NR_hdf5":
            self.injection_arguments["waveform_approximant"] = self.injection_options[
                "approximant"
            ]
            self.injection_arguments["numerical_relativity_file"] = (
                self.injection_options["numrel_path"]
            )
        ...
        self.injection_parameters.update({"luminosity_distance": 64.0})

        self.ifos.set_strain_data_from_zero_noise(
            sampling_frequency=self.sampling_frequency,
            duration=self.duration,
            start_time=self.injection_parameters["geocent_time"]
            - (self.duration - 2.0),
        )
        self.ifos.inject_signal(
            waveform_generator=self.injection_generator,
            parameters=self.injection_parameters,
        )

        # Calculate correct luminosity distance to achieve targeted network SNR
        candidate_network_snr = np.sqrt(
            np.sum([ifo.meta_data["optimal_SNR"] ** 2 for ifo in self.ifos])
        )
    ...
    def initialize_waveform_generator(self):
        ...
        self.waveform_generator = get_waveform_generator(
            ...
            self.duration,
            self.recovery_options["f_low"],
            self.recovery_options["f_ref"],
        )
...
def _get_parameters_from_config_file(config_file):
    ...
    injection_parameters.update(
        {k: float(injection_parameters[k]) for k in ["f_low", "f_ref", "snr"]}
    )
    cbc_parameters = {a: float(b) for a, b in cbc_parameters.items()}
    recovery_parameters.update(
        {k: float(recovery_parameters[k]) for k in ["f_low", "f_ref"]}
    )
    recovery_parameters.update(
        {k: int(recovery_parameters[k]) for k in ["ntemps", "nsamples"]}
    )
...
def _get_hyper_prior(parameter_dict):
    ...
    mass_1_list, mass_2_list = utils.mchirp_q_to_mass1_mass2(*chirp_mass_q_prior.T)
    ...
    hyper_prior = np.append(
        hyper_prior, np.array([[mass_1, mass_2, chi_1, chi_2]]), axis=0
    )
    return hyper_prior
...
def get_duration(parameter_dict, f_low):
    ...
    durations = utils.get_imr_duration(
        *hyper_prior.T, f_low=f_low, approximant="IMRPhenomD"
    )
